...
--
FSMB_toonlist={[1]="Vato",[2]="Hato",[3]="Gato",[4]="Tato",[5]="Sato"}
FSMB_invitelist={[1]="Vato-thalnos",[2]="Hato-thalnos",[3]="Gato-thalnos",[4]="Tato-thalnos",[5]="Sato-thalnos"}
FSMB_tank="Vato"
FSMB_clothto="Vaj"
FSMB_tradeopen=nil
FSMB_nomacros=nil
...
function init()
	...
	for i=1,TableLength(FSMB_toonlist) do
		...
		if myname==FSMB_toonlist[i] then
			macroId = CreateMacroFS("f"..i.."_fs", "Ability_HUNTER_pet_gorilla", "/run SetView(4)", nil);
else
	...
	if not FSMB_nomacros then
		...
		if myClass=="PALADIN" or myClass=="SHAMAN" or myClass=="DRUID" or myClass =="MONK" or myClass =="PRIEST" or myClass=="DEATHKNIGHT" then
			if FSMB_myrez[myClass] then
				macroId=CreateMacroFS("REZ_FS_"..myspec,"spell_holy_resurrection","#showtooltip\n/click [button:1] REZ_FS_"..myspec.." LeftButton t;",nil)
				PickupMacro(macroId)
				PlaceAction(8)
				ClearCursor()
			end
		...
		ClearCursor()
		if myClass=="HUNTER" then 
			index=CreateMacroFS("RSINGLE_FS_"..myspec,"ability_searingarrow","#showtooltip\n/click [button:1] RSINGLE_FS_"..myspec.." LeftButton t;",nil)
			PickupMacro(index)
			PlaceAction(14)
			ClearCursor()
		end
		...
		ClearCursor()
		if myClass=="HUNTER" then 
			index=CreateMacroFS("RMULTI_FS_"..myspec,"ability_searingarrow","#showtooltip\n/click [button:1] RMULTI_FS_"..myspec.." LeftButton t;",nil)
			PickupMacro(index)
			PlaceAction(15)
			ClearCursor()
		end
...
MB_allmactypes = {"REZ_FS_","SETUP_FS_","MULTI_FS_","TURBO_FS_","AOE_FS_","SHEAL_FS_","SINGLE_FS_","single_fs_","SETUP_FS_","multi_fs_","turbo_fs_","aoe_fs_","sheal_fs_"}
